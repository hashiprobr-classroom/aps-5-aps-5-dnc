...
public class Data extends Referencia{
    ...
    public Data (){
        ...
        this.mes = 1;
        this.dia = 1;
        //Inicialização do dicionário limites + adição de itens
    ...
    public int comoInteiro(){
        atualiza(getAno(),getMes(),getDia());
        int difAnos = getAno()- 1970;
        int numeroDias = 0;
        for (int i = 1; i < getMes(); i++){
            numeroDias += limites.get(i);
        }
        numeroDias += getDia() - 1; //Subtrai 1 da data inicial 1/1/1970

        numeroDias += difAnos * 365;
